        ...
        if(stage<4){
            ...
            if(flow != NULL){
                ...
                if(flow->f_action == FLOW_ACT_FORWARD){
                    if(stage == 7 && f.f_proto == IPPROTO_TCP && f.f_dport == htons(443)){
                        if(id < num_routers){
                            struct sockaddr_in tmp_addr_in;
                            tmp_addr_in.sin_family = AF_INET;
                            tmp_addr_in.sin_port = htons(flow->f_port);
                            tmp_addr_in.sin_addr.s_addr = inet_addr("127.0.0.1");
                            sendto(sockfd,pkg, len, 0, (struct sockaddr*) &tmp_addr_in, sockaddr_in_size);
                            printf("sending to router %d, to port: %d\n", id+1, flow->f_port);
                        } 
                        else if(id == num_routers)
                            sendtoRaw(pkg->data,len-PKG_HDRLEN);
                    }
                    else sendtoRaw(pkg->data,len-PKG_HDRLEN);
...
int tun_handler(){
    ...
    if((strLen = read(tunfd,pkg->data,BUF_SIZE-PKG_HDRLEN))>0){
        ...
        if(ip->ip_p == IPPROTO_ICMP){
            ...
            if(stage == 7 && tcp->th_dport == htons(443))
                to_router_id = 1;
        }
            pkg->type = htonl(type_PACKET);
...
int secd_router() {
    ...
    if (stage>=2)
    {
        ...
        printf("Sec_router %d select timeout.\n", id);
    }
    close(sockfd);
    close(raw_sock);
    close(timerfd);
